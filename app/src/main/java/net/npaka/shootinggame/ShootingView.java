...
public class ShootingView extends SurfaceView
    ...
    public ShootingView(Activity activity) {
        ...
        segmentFont = Typeface.createFromAsset(getContext().getAssets(),"7SegmentsLED.ttf");
        nicoFont = Typeface.createFromAsset(getContext().getAssets(),"nicomoji_1.00.ttf");
    }
    ...
    public void drawScore() {
        ...
        g.drawText(num2str(score, 6),140+g.getOriginX(), 10-(int)g.getFontMetrics().ascent);
        g.setScoreTypeface(nicoFont);
    }
    ...
    public void run() {
        ...
        while(thread != null) {
            ...
            if (scene == S_PLAY) {
                //隕石の出現(2)
                tick++;
                if (tick > 5) {//隕石の数　ループ数を下げると隕石の数増える
                    tick = 0;
                ...
                for (int i = shots.size()-1; i >= 0; i--) {
                    ...
                    //pos0.y -= 10;
                    pos0.y -=5;//移動速度半分(y(縦)方向へ５ずつ移動していく)
            ...
            for (int i = shots.size()-1; i >= 0; i--) {
                ...
                g.drawBitmap(bmp[7], pos.x-60, pos.y-18);
                g.drawBitmap(bmp[7], pos.x+40, pos.y-18);
            }
            ...
            //offsetTime[a] = endTime - startTime;
            //a++;
            //long average = (offsetTime[a] + offsetTime[a+1]) /offsetTime[a];
            ...
            g.setTextSize(30);
            drawScore();
            g.drawText(num2str((int)offsetTime,2) ,400, 10+g.getOriginY()-(int)g.getFontMetrics().ascent);
            //g.drawText(num2str(score, 6),10, 10+g.getOriginY()-(int)g.getFontMetrics().ascent);
            g.drawText("SCORE ",10+g.getOriginX(), 10-(int)g.getFontMetrics().ascent);
            g.unlock();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (touchAction == MotionEvent.ACTION_DOWN) {
            ...
            //プレイ
                ...
                //弾の追加(3)
                shots.add(new Point(shipX, shipY-10));//弾の追加される位置
